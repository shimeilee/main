...
This guide will provide _____ /
D

//how to use developer guide
=== Symbols
Before proceeding, do keep in mind the below call out signs.

[WARNING]
This callout indicates that there is something worth noting.

[NOTE]
Key information that you should take note of.

[TIP]
Helpful tips that can prove to be really handy for you!
...
user that will delete the homework entry at the index which has entered.
Any invalid format of the command will be prompted by the command parser.

==== Current Implementation

The delete homework feature is executed by the `DeleteHomework` Command. Currently, the
...
complexities involved when we have to take into account the different cases when using deletion by homework name.
// end::deletehomework[]

// start::edithomework[]
=== Edit homework feature

This feature allows the user to edit any attributed of the homework entries. This is a core feature because the user
...
'mc/' for `ModuleCode`,'hw/' for `HomeworkName` and 'd/' for `Date`. A validation check will be impose upon editing
of each object. Any checks that fails the validation would prompt the user on the failed component.
...
and proceeds to alter the attributes of the homework entry in the given index.
...
which are illustrated by the blue arrows point from the command parsers back to the LogicManger.
...
*Step 4*: The user executes `edit-cap 2 g/A sem/Y2S2` to edit the cap entry in the 2nd index with grade A and
moduleSemester of `Y2S2`.

*Step 5*: The user executes `edit-cap 2 g/A sem/Y2S2` to edit the cap entry in the 2nd index with grade A and
moduleSemester of `Y2S2`.

*Step 6*: The user executes `edit-cap 2 g/A sem/Y2S2` to edit the cap entry in the 2nd index with grade A and
moduleSemester of `Y2S2`.
...
image::DeleteCapEntryActivityDiagram.png[width="800"]
